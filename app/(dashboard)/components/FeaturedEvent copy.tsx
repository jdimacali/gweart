...
import { parseISO, addDays, isAfter, isWithinInterval } from "date-fns";
  ...
  useEffect(() => {
    const getNextEvent = async () => {
      try {
        ...
        const getEventTiming = (event: Events) => {
          const startDate = toZonedTime(
            parseISO(event.attributes.start_date),
            timeZone
          );
          const endDate = event.attributes.end_date
            ? toZonedTime(parseISO(event.attributes.end_date), timeZone)
            : startDate;
          const dayAfterEnd = addDays(endDate, 1);

          if (isAfter(now, dayAfterEnd)) {
            return "past";
          }
        ...
        const categorizedEvents = events.reduce(
          (
            acc: { happening: Events[]; upcoming: Events[]; past: Events[] },
            event
          ) => {
            const timing = getEventTiming(event);
            acc[timing].push(event);
            return acc;
          },
          {
            happening: [],
        ...
        categorizedEvents.upcoming.sort((a, b) => {
          const startA = parseISO(a.attributes.start_date || "");
          const startB = parseISO(b.attributes.start_date || "");
          return startA.getTime() - startB.getTime();
        });

        categorizedEvents.happening.sort((a, b) => {
          const endA = parseISO(
            b.attributes.end_date || b.attributes.start_date || ""
          );
          const endB = parseISO(
            a.attributes.end_date || a.attributes.start_date || ""
          );
          return endA.getTime() - endB.getTime();
        });

        const nextEvent = [
          ...categorizedEvents.happening,
